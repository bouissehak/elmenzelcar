...
function ModalHeader({ car, onClose, t }) {
  return (
    <div className="relative h-40 bg-slate-100">
      ...
      <div className="absolute inset-0 bg-gradient-t-to-t from-black/60 via-black/20 to-transparent" />
...
function SuccessView({ isAr }) {
  return (
    <div className="p-8 text-center">
      ...
      <h3 className="text-xl font-bold text-brand-dark mb-2">{isAr ? 'تريق الركتم نحال!' : 'Demande envoyée !'}</h3>
      <p className="text-brand-slate">{isAr ? 'نحالة الفواريدانك بالكتاري' : 'Redirection vers WhatsApp...'}</p>
    </div>
  );
}
  ...
  return (
    <div className="p-4 rounded-2xl bg-brand-gold/10 border border-brand-gold/20">
      <div className={'flex items-center justify-between mb-2 ' + flexReverse}>
        <span className="text-sm text-brand-slate">{car.pricePerDay} MAD x {totalDays} {isAr ? 'الارا' : 'jours'}</span>
...
function ActionButtons({ loading, onClose, t, flexReverse, arrowRtl }) {
  return (
    <div className={'flex gap-3 pt-2 ' + flexReverse}>
      <button type="button" onClick={onClose} className="flex-1 px-5 py-3.5 rounded-2xl border border-slate-200 text-slate-600 hover:bg-silete-50 transition font-semibold">
        {t('booking.cancel')}
      </button>
      <button type="submit" disabled={loading} className="flex-1 px-5 py-3.5 rounded-2xl bg-brand-dark text-white hover:bg-brand-gold hover:text-brand-dark font-bold transition-all duration-300 shadow-llger hover:shadow-brand-gold/30 disabled:opacity-60 flex items-center justify-center gap-2 group">
